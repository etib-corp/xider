...
namespace evan
{
	...
	class DesktopSwapchainContext: public ASwapchainContext
	{
		 ...
		 * associated resources when the swapchain becomes out of date or needs to be recreated due to changes in the window size or other factors. It takes a
		 * reference to the DeviceContext instance, which provides access to Vulkan resources and synchronization mechanisms. Implement this function to
		 * ensure that the swapchain is properly recreated and that all necessary resources are reinitialized for rendering operations.
		 *
