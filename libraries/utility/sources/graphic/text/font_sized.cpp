...
namespace utility::graphic
{
	...
	Glyph FontSized::generateGlyph(uint32_t codePoint)
	{
		...
		glyph.uvMin = { (float)_penX / _atlasWidth,
						(float)_penY / _atlasHeight };

		glyph.uvMax = { (float)(_penX + g->bitmap.width) / _atlasWidth,
	...
	std::shared_ptr<Texture> FontSized::getAtlas(bool shouldRegenerate)
	{
		if (shouldRegenerate || !_generatedAtlas) {
			if (!_generatedAtlas)
				_generatedAtlas =
					std::make_shared<Texture>(_atlasWidth, _atlasHeight);
			generateAtlas();
		}
